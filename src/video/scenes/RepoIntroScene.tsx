...
export const RepoIntroScene: React.FC<RepoIntroSceneProps> = ({
  ...
  // Stats bounce in with delay
  const statsSpring = spring({ frame: frame - 8, fps, config: springs.bouncy });
  const statsScale = interpolate(statsSpring, [0, 1], [0.5, 1]);
  const statsOpacity = interpolate(frame, [8, 18], [0, 1], {
    extrapolateRight: 'clamp',
  });
  ...
  const getAnimatedValue = (target: number, startFrame: number, duration: number = 30) => {
  ...
  const animatedStars = getAnimatedValue(stars, 20);
  const animatedForks = getAnimatedValue(forks, 25);
    ...
    >
      ...
      <div
        style={{
          display: 'flex',
          gap: 50,
          transform: `scale(${statsScale}) translateY(${exitY}px)`,
      ...
      >
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 18, color: '#888', marginTop: 4 }}>stars</div>
        </div>
        ...
        <div style={{ textAlign: 'center' }}>
          <div
            style={{
              ...
              fontWeight: 700,
              color: '#60a5fa',
              textShadow: `0 0 20px #60a5fa44`,
            }}
          >
            🍴 {formatNumber(animatedForks)}
          </div>
          <div style={{ fontSize: 18, color: '#888', marginTop: 4 }}>forks</div>
        </div>
      </div>
